...
class SessionManager {
  ...
  /// 旧业务 ID（U 前缀）。
  /// 2026-09-14 双 ID 统一后 currentUserId ≡ auth uuid；此字段仅用于把升级前
  /// 持久化会话里的旧 app_user_id 交给 GameCumulativeService 做一次性本地键
  /// 迁移（内存态，不持久化、不参与任何业务请求）。
  String? _legacyBusinessId;
  ...
  /// 当前用户唯一标识 = auth.users.id（uuid）。
  ///
  /// 2026-09-14 双 ID 统一：public.users.id 已与 auth uuid 合一（迁移
  /// migrate_unify_user_id_v1），RLS（get_user_business_id() ≡ auth.uid()）
  /// 与本值天然一致；旧 metadata.app_user_id 口径废弃。
  String? get currentUserId => _authUser?['id'] as String?;

  /// 升级前会话里的旧业务 ID（U 前缀），仅供本地累计计数一次性迁移；无则 null。
  String? get legacyBusinessId => _legacyBusinessId;
  ...
  Future<bool> restoreSession() async {
    try {
      ...
      if (token != null && userJson != null) {
        ...
        // 捕获升级前会话里的旧业务 ID（U 前缀），供 game_cum 本地键一次性迁移；
        // 重新登录/刷新用户信息后该值不复存在，故仅在恢复持久化会话时取一次
        _legacyBusinessId =
