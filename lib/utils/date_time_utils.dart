...
/// - TIMESTAMPTZ: created_at, updated_at, remind_at - 存储完整时间戳
/// - DATE: date (expenses, mood_diaries, weight_records) - 仅存储日期 YYYY-MM-DD
class DateTimeUtils {
  ...
  static String formatStandard(DateTime? dateTime) {
    ...
    return _standardFormat.format(dateTime.toLocal());
  }
  ...
  /// 解析 DATE 字符串 (YYYY-MM-DD) 为 DateTime
  /// 适用于 expenses, mood_diaries, weight_records 的 date 字段
  ...
  static String nowStandard() {
    return formatStandard(DateTime.now());
  }
}
