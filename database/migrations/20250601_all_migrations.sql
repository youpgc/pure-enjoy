...
-- 2. 小说章节表 (V1.8.7)
-- 用途: 小说阅读功能
-- 注意: 表可能已存在（管理后台创建），使用实际列名
-- novels: chapter_count(非chapter_count), is_free(非is_free)
-- chapters 实际表名: novel_chapters
-- ============================================================
...
-- 章节表（如已存在则跳过，实际表名 novel_chapters）
...
-- 示例小说1: 斗破苍穹
INSERT INTO novels (id, title, author, description, category, status, chapter_count, is_free)
VALUES (
...
-- 示例章节
INSERT INTO novel_chapters (id, novel_id, title, chapter_num, content, word_count) VALUES
...
SELECT '- novel_chapters (小说章节表)' AS table_name;
SELECT '- user_novels (用户书架关联表)' AS table_name;
